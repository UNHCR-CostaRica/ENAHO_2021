...
ind_21_nic <- poverty_nic$n/count(total_nic) 
ind_21_crc <- poverty_crc$n/count(total_crc)
...
access_nic <-ENAHO_2021_usuarios_%>%
  filter( LugNac == 2)%>%
  select(c(ID_HOGAR,REGION,ZONA,TamHog,LugNac,V12,V13A,V14A,V15,V16))%>%
...
ind_22_nic <- (sum(access_nic$access2_all_BS)/count(total_nic))
ind_22_crc <- (sum(access_crc$access2_all_BS)/count(total_crc))

#2.3 Proportion of PoC with access to health services

health_nic <-ENAHO_2021_usuarios_%>%
  filter( LugNac == 2)%>%
  select(c(ID_HOGAR,REGION,ZONA,TamHog,LugNac,A11))%>%
filter(A11 == 0)
...
ind_23_nic <- (1-(count(health_nic)/count(total_nic)))
ind_23_crc <- (1-(count(health_crc)/count(total_crc)))
...
nic_clean_fuels <- ENAHO_2021_usuarios_%>%
  select(.,c(ID_HOGAR,REGION,ZONA,TamHog,LugNac,V16))%>%
  mutate(.,clean_fuels = if_else(V16 == 0 | V16 == 3 | V16 == 4,0,1))%>%
  filter(.,LugNac == 2)
  
ind81_nic <- (sum(nic_clean_fuels$clean_fuels))/count(total_nic)
...
ind81_crc <- (sum(crc_clean_fuels$clean_fuels))/count(total_crc)
...
nic_unemployed <-ENAHO_2021_usuarios_%>%
  filter( LugNac == 2)%>%
  select(c(ID_HOGAR,REGION,ZONA,TamHog,LugNac,A5,B6,B7A,B7B,B7C,B7D,B7E,
...
ind133_nic <- (count(nic_unemployed)/count(nic_work_force))
...
ind133_crc <- (count(crc_unemployed)/count(crc_work_force))

# and now the graph

pop3 <- c("Costa Ricans", "Nicaraguans","Costa Ricans", "Nicaraguans")
ind3 <- c("rbm8.1","rbm8.1","rbm13.3","rbm13.3")
est3 <- c(ind81_crc,ind81_nic,ind133_crc,ind133_nic)
